...
export default function CheckEx1(props: CheckBoxProps) {
	...
	return (
		...
		// 	<input id={id} className={"peer sr-only"} {...rest} />
		// 	<span
		// 		className={twMerge(
		...
		// 		</svg>
		// 	</span>
		// 	<span className="message text-white font-medium">{children}</span>
		// </label>
		<div className="center gap-2">
			<input
				id={id}
				className="appearance-none w-5 h-5 rounded-[5px] bg-gray border bd-gray checked:bg-[url('../assets/check.svg')] checked:bg-no-repeat checked:bg-center"
				{...rest}
			/>
			<label htmlFor={id} className="message text-white font-medium">
				{children}
			</label>
		</div>
	);
